  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        "hover:shadow-[0_8px_30px_rgba(0,0,0,0.06),0_0_0_1px_rgba(99,102,241,0.08)]",
    ...
    >
      ...
      >
        <div className="absolute inset-0 animate-shine bg-gradient-to-r from-transparent via-white/10 to-transparent" style={{ animationPlayState: 'paused' }} />
      </div>
...
const DetailRow = ({
  ...
  >
    ...
    <span className="text-gray-500 flex items-center gap-1.5 text-sm group-hover/row:text-gray-600 transition-colors">
      {Icon && <Icon className="w-3.5 h-3.5 group-hover/row:text-blue-500 transition-colors duration-300" />}
      {label}
    </span>
    <span className="font-medium text-right text-gray-700 text-sm">{value}</span>
  </motion.div>
);
...
export default function ClientPortal() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40">
        <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
          ...
          {[1,2].map(i=>(
            <div key={i} className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 space-y-4">
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-red-50/20 to-slate-50 flex items-center justify-center p-4">
        <motion.div
          ...
          className="bg-white/90 backdrop-blur-xl border border-red-100 rounded-2xl shadow-xl shadow-red-500/5 max-w-md w-full p-8 text-center"
        >
          <motion.div
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">{t('common.error', lang)}</h2>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-blue-50/20 to-indigo-50/30 relative">
      {/* Background decorative elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          animate={{ x: [0, 30, 0], y: [0, -20, 0] }}
          transition={{ duration: 25, repeat: Infinity, ease: 'linear' }}
          className="absolute -top-40 -right-40 w-96 h-96 bg-blue-200/15 rounded-full blur-3xl"
        />
        <motion.div
          animate={{ x: [0, -20, 0], y: [0, 30, 0] }}
          transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
          className="absolute -bottom-40 -left-40 w-96 h-96 bg-indigo-200/15 rounded-full blur-3xl"
        />
        <motion.div
          animate={{ scale: [1, 1.1, 1] }}
          transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
          className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-purple-100/8 rounded-full blur-3xl"
        />
        {/* Subtle grid pattern */}
      ...
      {/* ============ HEADER ============ */}
      <header className="relative z-10 bg-white/60 backdrop-blur-xl border-b border-gray-200/40">
        <div className="max-w-3xl mx-auto px-4 py-4 flex justify-between items-center">
          <Link to="/" className="no-underline">
            ...
            >
              ...
              <div>
                <h1 className="text-lg font-bold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-600 bg-clip-text text-transparent group-hover/logo:from-blue-700 group-hover/logo:via-indigo-600 group-hover/logo:to-purple-600 transition-all duration-500">
                  FibreConnect
                </h1>
                <p className="text-xs text-gray-400 group-hover/logo:text-gray-500 transition-colors">{t('client.welcome', lang)}</p>
              </div>
            </motion.div>
          </Link>
          ...
          <motion.div
            ...
            className="flex items-center gap-1.5 bg-gray-100/70 backdrop-blur-sm rounded-xl p-1 border border-gray-200/50"
          >
            ...
            {(['fr', 'de', 'en'] as const).map(l => (
              ...
              >
                {lang === l && (
                  <motion.div
                    layoutId="activeLang"
                    className="absolute inset-0 bg-white shadow-sm shadow-gray-200/50 rounded-lg border border-gray-100"
                    transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                  />
                )}
                <span className={cn(
                  "relative z-10 transition-colors duration-200",
                  lang === l ? 'text-blue-600 font-semibold' : 'text-gray-500 hover:text-gray-700'
                )}>
                  {l.toUpperCase()}
                </span>
              </button>
            ...
            <StaggerItem>
              <motion.div className="text-center space-y-1">
                <motion.span
                  ...
                  className="inline-flex items-center gap-1.5 text-xs text-gray-400 tracking-widest uppercase font-medium"
                >
            ...
            <AnimatePresence>
              {actionMsg && (
                <motion.div
                  ...
                  className="relative bg-emerald-50/80 backdrop-blur-sm border border-emerald-200/60 rounded-2xl p-4 text-emerald-700 text-sm text-center flex items-center justify-center gap-2 overflow-hidden"
                >
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    ...
                    {steps.map((s, i) => {
                      ...
                      return (
                        <React.Fragment key={i}>
                          ...
                          >
                            ...
                            <motion.span
                              ...
                              className={cn(
                                ...
                                isHovered && !isActive && 'text-gray-500',
                              )}
                            >
                              {s.label}
                            </motion.span>
                          ...
                          {/* Connector bar */}
                          {i < steps.length - 1 && (
                            <div className="flex-1 h-[3px] mx-2 rounded-full bg-gray-100 overflow-hidden relative">
                              <motion.div
            ...
            {isCancelled && (
              <StaggerItem>
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  className="relative bg-gradient-to-br from-red-50/90 to-red-100/50 backdrop-blur-lg border border-red-200/50 rounded-2xl p-8 text-center shadow-lg shadow-red-500/5 overflow-hidden"
                >
              ...
              <TiltCard>
                ...
                <div className="space-y-0.5">
                  <DetailRow
                    ...
                    value={
                      <AnimatedBadge className="bg-blue-50 text-blue-700 px-3 py-1 border border-blue-100/50">
                  ...
                  <DetailRow
                    ...
                    value={
                      <AnimatedBadge className={cn(
                        "px-3 py-1 border",
                        data.status === 'finalisee' ? 'bg-emerald-50 text-emerald-700 border-emerald-100/50' :
                        data.status === 'en_retard' ? 'bg-red-50 text-red-700 border-red-100/50' :
                        data.status === 'en_cours' ? 'bg-amber-50 text-amber-700 border-amber-100/50' :
                ...
                <TiltCard>
                  ...
                  <div className="flex items-center gap-4 group/tech">
                    ...
                    <div className="flex-1">
                      <p className="font-semibold text-gray-900 group-hover/tech:text-indigo-700 transition-colors duration-300">
                      ...
                      {data.technician.phone && (
                        <motion.a
                          ...
                          whileHover={{ x: 3 }}
                          className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-700 transition-colors mt-1 group/phone"
                        >
                  ...
                  {isInProgress && data.technician.current_latitude && (
                    <motion.div
                      ...
                      className="mt-5 p-4 bg-gradient-to-br from-emerald-50/90 to-teal-50/70 backdrop-blur-sm border border-emerald-200/50 rounded-2xl"
                    >
                      ...
                      <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 border border-emerald-100/50">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-500">{t('client.eta', lang)}</span>
              ...
              <StaggerItem>
                <TiltCard className="relative overflow-hidden">
                  <div className="absolute inset-0 opacity-20 pointer-events-none">
                    <SparklesCore particleCount={20} particleColor="#8b5cf6" minSize={0.3} maxSize={0.8} />
                  </div>
                  <div className="relative z-10">
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-3 bg-white/70 backdrop-blur-sm border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 outline-none transition-all resize-none hover:border-gray-300"
                      rows={3}
              ...
              <StaggerItem>
                <TiltCard className="text-center relative overflow-hidden">
                  <div className="absolute inset-0 opacity-15 pointer-events-none">
                    <SparklesCore particleCount={12} particleColor="#f59e0b" minSize={0.3} maxSize={0.7} />
                  </div>
                  <div className="relative z-10">
                    <h3 className="font-semibold text-gray-900 mb-4">{t('client.rate_service', lang)}</h3>
                    ...
                    <p className="text-sm text-gray-500 font-medium">{data.client_rating}/5</p>
                    {data.client_comment && (
                      <motion.div
                        initial={{ opacity: 0, y: 8 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.5 }}
                        className="mt-4 text-sm text-gray-600 italic bg-gray-50/60 rounded-xl p-4 border border-gray-100/50 relative"
                      >
                        <span className="absolute top-2 left-3 text-2xl text-gray-200 font-serif">"</span>
                        <span className="relative z-10 ml-4">{data.client_comment}</span>
                        <span className="absolute bottom-1 right-3 text-2xl text-gray-200 font-serif">"</span>
                      </motion.div>
                    )}
                  </div>
                </TiltCard>
      ...
      >
            ...
            <motion.label
              ...
              className={cn(
                ...
                selectedSlot === `${s.date}|${s.start}`
                  ? 'border-blue-400 bg-blue-50/80 shadow-sm shadow-blue-500/5'
                  : 'border-gray-200 hover:bg-gray-50/80 hover:border-gray-300 hover:shadow-sm'
              )}
            >
              <input
              ...
              />
              <div className="flex-1">
                <p className="font-medium text-sm text-gray-900">{s.date}</p>
                <p className="text-xs text-gray-500">{s.start} - {s.end}</p>
              </div>
              <ChevronRight className="w-4 h-4 text-gray-300 opacity-0 group-hover/slot:opacity-100 transition-opacity" />
            </motion.label>
          ))}
        </div>
        <textarea
          value={comment} onChange={e => setComment(e.target.value)}
          className="w-full mt-4 px-4 py-3 bg-white/70 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 outline-none transition-all resize-none hover:border-gray-300"
          rows={2}
        ...
        <div className="flex gap-3 mt-4">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
            ...
            className="flex-1 py-2.5 px-4 bg-gray-100 hover:bg-gray-200 rounded-xl font-medium text-sm text-gray-700 transition-colors"
      ...
      >
        <textarea
          value={comment} onChange={e => setComment(e.target.value)}
          className="w-full px-4 py-3 bg-white/70 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 outline-none transition-all resize-none hover:border-gray-300"
          rows={4}
        ...
        <div className="flex justify-center gap-1.5 mt-2">
          {['DE', 'BE', 'FR', 'LU'].map((c, i) => (
            <motion.span
              ...
              className="text-[10px] font-bold text-gray-400 bg-gray-100/60 px-2 py-0.5 rounded cursor-default hover:text-blue-500 hover:bg-blue-50/60 transition-colors duration-200"
            >
              {c}
            </motion.span>
          ))}
